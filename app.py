...
from static.models import db, POSTS, USERS, QUEUE, app
...
async def about():
    ...
    return jsonify({
        ...
        "text": 'Меня зовут Егор Жижло, и я являюсь начинающим Backend-разработчиком.'
                ' В данный момент обучаюсь на 1 курсе факультета Прикладной математики и'
                ' информатики Финансового Университета при Правительстве РФ. Этот блог был'
                ' реализован с помощью REST API с использованием фреймворка Flask для того,'
                ' чтобы получить новые навыки и знания в сфере е только в Backend-разработки, но и других.',
        "contacts": {
...
@app.route('/gradient/date/', methods=['GET', 'POST'])
async def gradientdate():
    operation_key = request.json
    if 'operation_key' not in operation_key or not operation_key:
        return jsonify({'data': 0})
    if not check_password_hash(operation_key['operation_key'], o_key):
        return jsonify({'data': 0})
    last = QUEUE.query.all()
    if len(last) != 0:
        return jsonify({'data': 1, 'date': last[0].date})
    else:
        return jsonify({'data': -1, 'date': 0})


@app.route('/gradient/start/', methods=['GET', 'POST'])
async def gradientstart():
    operation_key = request.json
    if 'operation_key' not in operation_key or not operation_key or 'date' not in operation_key:
        return jsonify({'data': 0})
    if not check_password_hash(operation_key['operation_key'], o_key):
        return jsonify({'data': 0})
    QUEUE.query.delete()
    db.session.add(QUEUE(date=operation_key['date']))
    db.session.commit()
    return jsonify({'date': 1})
